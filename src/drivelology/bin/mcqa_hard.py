#!/usr/bin/env python3
"""
Multiple Choice Question Answering (MCQA) for narrative classification - Hard version.

This script runs experiments to evaluate how well language models can
identify when none of the provided narratives match the text.
"""
...
from drivelology.mcqa.config import PROMPT_CONFIGS_HARD
from drivelology.mcqa.models import generate_prediction
from drivelology.mcqa.data import (
    get_narrative_fields, load_existing_results, prepare_options_hard, 
    save_result_hard, load_dataset_split
)
...
def main():
    """Main execution function for MCQA hard version experiments."""
    ...
    if prompt_version not in PROMPT_CONFIGS_HARD:
        raise ValueError(f"Unknown prompt version: {prompt_version}. Available: {list(PROMPT_CONFIGS_HARD.keys())}")
    
    prompt_config = PROMPT_CONFIGS_HARD[prompt_version]
    console.log(f"Using prompt: {prompt_config.name} ({prompt_config.description})")
    
    # Setup save file
    save_dir = setup_output_directory(dir_name='outputs/mcqa_hard')
    ...
    for idx, row in enumerate(data):
        ...
        # Prepare options (only negative narratives for hard version)
        options, correct_answer = prepare_options_hard(row, neg_fields)
        ...
        # Save result
        save_result_hard(save_file, row, options, correct_answer, prediction)
